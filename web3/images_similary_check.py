...
if __name__ == '__main__':
    ...
    vector_index_file_names_path = resouces_folder+"index_to_file_name.json"
    index_file_path = resouces_folder+'hnswlib.bin'
    vectors_folder = resouces_folder+"vectors_test_data\\*.npz"
    features_batches_folder = resouces_folder+"vector_features_batches\\*"
    ...
    if path.exists(vector_features_file_path):
        with open(vector_features_file_path, "r") as read_file:
            data_set = list(numpy.asarray(json.load(read_file)['data']))
        vector_features_batches_glob = glob.glob(features_batches_folder)
        for batch_file_path in vector_features_batches_glob:
            try:
                with open(batch_file_path, "r") as read_file:
                    in_file_data_set = list(np.asarray(json.load(read_file)['data']))
                    data_set = data_set + in_file_data_set
            except Exception as e:
                print(e)
                continue
        with open(vector_index_file_names_path, "r") as read_file:
            index_to_file_name = json.load(read_file)
        # index_to_file_name = json.loads(vector_index_file_names_path)
    else:
        all_files = glob.glob(vectors_folder)
        total_files = len(all_files)
        print(total_files)
        load_glob_worker(all_files,index_to_file_name,data_set,0,total_files,lock)
    ...
    if path.exists(index_file_path) == False:
        ...
        index.load_index(index_file_path,max_elements = total_files)
    ...
    #     index_to_file_name[total_files+new_feature_index] = second_file_name_index[new_feature_index]
    ...
    for master_index, master in enumerate(distances):
        ...
        for index_of_distance_to_neighbor, distance_to_neighbor in enumerate(master):
            ...
            if distance_to_neighbor <= 0.02:
                master_file_path = index_to_file_name[str(master_index)]
                neighbor_index = labels[master_index][index_of_distance_to_neighbor]
                neighbor_file_path = index_to_file_name[str(neighbor_index)]
